define([
  "esri/Graphic",
  "esri/tasks/QueryTask",
  "esri/tasks/support/Query",
], function (
  Graphic,
  QueryTask,
  Query
) {

  var __idgrilla = '',
    __idtable = '',
    __datatable = '',
    __arr_fieldhide = ["FOTO", 'IMAGEN', 'FICHA', 'ID_CG', 'ID_LT', 'ID_SET', 'DOCUMENTO']; //ID_CG ID_SET:ocultar  

  function loadTable(response, titlereporte, idtable, isexportable) {
    __idgrilla = '#' + $(idtable).parents('.grilla').attr('id');
    __idtable = idtable;
    $('#div_results .grilla').addClass('notvisible');
    hideGrid();

    let data = response.features,
      nreg = data.length,
      fields = response.fields,
      tbody = '',
      nfieldresponse = '0',
      nfield = fields.length,
      theader = '',
      aux_theader = '<th>#</th>',
      aligncolum = [],
      textalign = '';

    nreg != 0 ? nfieldresponse = Object.keys(data[0].attributes).length : ''; //saber si hay data que mostrar o no

    // limpiar tabla
    const rowcount = $(`${__idtable} > tbody tr`).length;
    if (rowcount > 0) {
      $(__idtable).DataTable().clear();
      $(__idtable).DataTable().destroy();
      $(`${__idtable} > thead`).html('');
      $(`${__idtable} > tbody`).html('');
    }

    // armar cabecera theader
    for (let j = 0; j < nfield; j++) {
      const columname = (fields[j].alias).toUpperCase();
      const typedata = fields[j].type;
      let textaligncolum = 'L';

      if (['double', 'small-integer', 'integer', 'single'].includes(typedata)) {
        textaligncolum = 'R';
      }

      if ($.inArray(fields[j].name, __arr_fieldhide) == -1) {

        if (typedata == 'oid') {
          aux_theader += `<th>Zoom</th>`;
          textaligncolum = 'C';
        } else {
          aux_theader += `<th>${columname}</th>`;
        }

      }

      aligncolum.push(textaligncolum);

    }

    theader = `<tr>${aux_theader}</tr>`;

    if (nreg != 0 && nfieldresponse != 0) {
      // armar cuerpo tbody
      for (let i = 0; i < nreg; i++) {
        const row = data[i].attributes;
        let aux_tbody = '<td></td>';
        for (let j = 0; j < nfield; j++) {
          let item = row[fields[j].name];
          const typedata = fields[j].type;

          // alineacion de los campos
          textalign = 'text-left';
          if (['double', 'small-integer', 'integer', 'single'].includes(typedata)) {
            textalign = 'text-right';
          }

          if ($.inArray(fields[j].name, __arr_fieldhide) == -1) {

            if (item == undefined || item === null) {
              aux_tbody += `<td> </td>`;
            } else {
              // dar formato a la fecha
              if (typedata == 'date') {
                item = getFormatDate(item, 'DD/MM/YYYY');
              }

              // dar formato a numeros doubles
              if (typedata == 'double') {
                item != null ? item = Number( item ).toFixed(2): '';
              }

              // solamente para campo con la palabra MES 
              if (fields[j].name == 'MES') {
                item = getMonthName(item).f;
              }

              // asigno la zoom 
              if (typedata == 'oid') {
                aux_tbody += `<td class='tdzoom' id="${item}" data-namefield="${fields[j].name}" data-toggle="tooltip" data-placement="left" title="Acercar" ><span class='esri-icon-zoom-in-magnifying-glass' ></span></td>`;
              } else {
                aux_tbody += `<td class='${textalign}'>${item} </td>`;
              }
            }
          }
        }
        tbody += `<tr>${aux_tbody}</tr>`;
      }
    } else {
      isexportable = false;
    }

    // luego llenar tabla con nueva data
    $(`${__idtable} > thead`).html(theader);
    $(`${__idtable} > tbody`).html(tbody);
    $(`${__idgrilla} .lbl-titlereporte`).text(titlereporte);

    // mostrar tabla
    showGrid();
    if ($('#div_results .grilla').hasClass('max-size')) {
      $('#div_results .grilla').removeClass('max-size').addClass('min-size');
    }

    // obtengo el tamaño para tbody del datatable 
    let lengthscrolly = getLengthScrollDataTable();

    // formateo con datatable
    let table = '';

    if (isexportable) {
      table = getDataTableExport(titlereporte, lengthscrolly, aligncolum);
    } else {
      table = getDataTableNotExport(lengthscrolly);
    }

    __datatable = table;
    hidePreloader();
  }

  function loadTableIndicador(response, titlereporte, idtable, isexportable) {
    __idgrilla = '#' + $(idtable).parents('.grilla').attr('id');
    __idtable = idtable;
    const aux_nameindicadores = ["FU", "TIFM", "TOLERANCIA"]; //para porcentajes
    $('#div_results .grilla').addClass('notvisible');
    hideGrid();
    let data = response.features,
      nreg = data.length,
      fields = response.fields,
      tbody = '',
      nfieldresponse = '0',
      nfield = fields.length,
      theader = '',
      aux_theader = '<th>#</th>';

    nreg != 0 ? nfieldresponse = Object.keys(data[0].attributes).length : ''; //saber si hay data que mostrar o no

    // limpiar tabla
    const rowcount = $(`${__idtable} > tbody tr`).length;
    if (rowcount > 0) {
      $(__idtable).DataTable().clear();
      $(__idtable).DataTable().destroy();
      $(`${__idtable} > thead`).html('');
      $(`${__idtable} > tbody`).html('');
    }

    // armar cabecera theader
    for (let j = 0; j < nfield; j++) {
      const columname = (fields[j].alias).toUpperCase();
      aux_theader += `<th>${columname}</th>`;
    }
    theader = `<tr>${aux_theader}</tr>`;

    if (nreg != 0 && nfieldresponse != 0) {
      // armar cuerpo tbody
      for (let i = 0; i < nreg; i++) {
        const row = data[i].attributes;
        let aux_tbody = '<td></td>';
        for (let j = 0; j < nfield; j++) {
          let item = row[fields[j].name];
          const typedata = fields[j].type;

          if (item == undefined || item === null) {
            aux_tbody += `<td> </td>`;
          } else {
            // dar formato a la fecha
            if (typedata == 'date') {
              item = getFormatDate(item, 'DD/MM/YYYY');
            }

            if (typedata == 'double') {
              item != null ? item = Number( item ).toFixed(2): '';
            }

            if (fields[j].name == 'MES') {
              item = getMonthName(item).f;;
            }
            if ($.inArray(fields[j].name, aux_nameindicadores) != -1) {
              item = (item * 100).toFixed(2) + '%';
            }
            aux_tbody += `<td>${item} </td>`;
          }
        }
        tbody += `<tr>${aux_tbody}</tr>`;
      }
    } else {
      isexportable = false;
    }

    // luego llenar tabla con nueva data
    $(`${__idtable} > thead`).html(theader);
    $(`${__idtable} > tbody`).html(tbody);
    $(`${__idgrilla} .lbl-titlereporte`).text(titlereporte);
    // mostrar tabla
    showGrid();
    if ($('#div_results .grilla').hasClass('max-size')) {
      $('#div_results .grilla').removeClass('max-size').addClass('min-size');
    }

    // obtengo el tamaño para tbody del datatable 
    const lengthscrolly = getLengthScrollDataTable();

    // formateo con datatable
    let table = getDataTable(lengthscrolly);
    __datatable = table;
    hidePreloader();
  }
  
  function loadTablePESuministro(response, fields, idtable) {
    __idtable = idtable;
    let data = response.features,
      nreg = data.length,
      tbody = '',
      nfieldresponse = '0',
      nfield = fields.length,
      theader = '',
      aux_theader = '<th>#</th>';

    nreg != 0 ? nfieldresponse = Object.keys(data[0].attributes).length : ''; //saber si hay data que mostrar o no

    // limpiar tabla
    let rowcount = $(`${__idtable} > tbody tr`).length;
    if (rowcount > 0) {
      $(__idtable).DataTable().clear();
      $(__idtable).DataTable().destroy();
      $(`${__idtable} > thead`).html('');
      $(`${__idtable} > tbody`).html('');
    }

    // armar cabecera theader
    for (let j = 0; j < nfield; j++) {
      const columname = (fields[j].alias).toUpperCase();
      aux_theader += `<th>${columname}</th>`;
    }
    theader = `<tr>${aux_theader}</tr>`;

    if (nreg != 0 && nfieldresponse != 0) {
      // armar cuerpo tbody
      for (let i = 0; i < nreg; i++) {
        const row = data[i].attributes;
        let aux_tbody = '<td></td>';
        for (let j = 0; j < nfield; j++) {
          let item = row[fields[j].name];
          const typedata = fields[j].type;

          if (item == undefined || item === null) {
            aux_tbody += `<td> </td>`;
          } else {
            // dar formato a la fecha
            if (typedata == 'date') {
              item = getFormatDatePE(item, 'DD/MM/YYYY HH:mm:ss');
            }
            if (typedata == 'double') {
              item != null ? item = Number( item ).toFixed(2): '';
            }
            aux_tbody += `<td>${item} </td>`;
          }
        }
        tbody += `<tr>${aux_tbody}</tr>`;
      }
    }

    // luego llenar tabla con nueva data
    $(`${__idtable} > thead`).html(theader);
    $(`${__idtable} > tbody`).html(tbody);

    // obtengo el tamaño para tbody del datatable 
    let lengthscrolly = null;

    // formateo con datatable
    let table = '';
    table = getDataTable(lengthscrolly);

    setTimeout(function () {
      table.columns.adjust().draw();
    }, 500);

    table.columns.adjust().draw();
    __datatable = table;
    hidePreloader();

  }

  function loadTablePE(response, fields, idtable, isactionlayerlist) {
    if (isactionlayerlist) {
      $('#div_results .grilla').addClass('notvisible');
      __idgrilla = '#' + $(idtable).parents('.grilla').attr('id');
      hideGrid();
    }

    __idtable = idtable;
    let data = response.features,
      nreg = data.length,
      tbody = '',
      nfieldresponse = '0',
      nfield = fields.length;

    nreg != 0 ? nfieldresponse = Object.keys(data[0].attributes).length : ''; //saber si hay data que mostrar o no

    // limpiar tabla
    let rowcount = $(`${__idtable} > tbody tr`).length;
    if (rowcount > 0) {
      $(__idtable).DataTable().clear();
      $(__idtable).DataTable().destroy();
      $(`${__idtable} > tbody`).html('');
      //tiene cabecera definida en el html 
    }

    if (nreg != 0 && nfieldresponse != 0) {
      // armar cuerpo tbody
      for (let i = 0; i < nreg; i++) {
        const row = data[i].attributes;
        let aux_tbody = '<td></td>';
        for (let j = 0; j < nfield; j++) {
          let item = row[fields[j].name];
          const typedata = fields[j].type;
          
          if (item == undefined || item === null) {
            aux_tbody += `<td> </td>`;
          } else {
            // dar formato a la fecha
            if (typedata == 'date') {
              item = getFormatDatePE(item, 'DD/MM/YYYY');
            }
            if (typedata == 'double') {
              item != null ? item = Number( item ).toFixed(2): '';
            }

            aux_tbody += `<td>${item} </td>`;
          }
        }
        tbody += `<tr>${aux_tbody}</tr>`;
      }
    } else {
      isexportable = false;
    }

    // luego llenar tabla con nueva data
    $(`${__idtable} > tbody`).html(tbody);

    // obtengo el tamaño para tbody del datatable 
    let lengthscrolly = null;

    if (isactionlayerlist) {
      // mostrar tabla
      showGrid();

      if ($('#div_results .grilla').hasClass('max-size')) {
        $('#div_results .grilla').removeClass('max-size').addClass('min-size');
      }

      lengthscrolly = getLengthScrollDataTable();

    }

    // formateo con datatable
    let table = '';
    table = getDataTable(lengthscrolly);

    __datatable = table;
    hidePreloader();

  }

  function loadTableTransform(response, titlereporte, idtable) {
    __idgrilla = '#' + $(idtable).parents('.card-tbltransf').attr('id');
    __idtable = idtable;
    hideGrid();
    let data = response.features,
      nreg = data.length,
      fields = response.fields,
      tbody = '',
      nfieldresponse = '0',
      nfield = fields.length,
      theader = '',
      aux_theader = '<th>#</th>',
      aligncolum = [],
      textalign = '';

    nreg != 0 ? nfieldresponse = Object.keys(data[0].attributes).length : ''; //saber si hay data que mostrar o no

    // limpiar tabla
    const rowcount = $(`${__idtable} > tbody tr`).length;
    if (rowcount > 0) { //no tiene datatable
      $(`${__idtable} > thead`).html('');
      $(`${__idtable} > tbody`).html('');
    }

    // armar cabecera theader
    for (let j = 0; j < nfield; j++) {
      const columname = (fields[j].alias).toUpperCase();
      const typedata = fields[j].type;
      let textaligncolum = 'L';

      if (['double', 'small-integer', 'integer', 'single'].includes(typedata)) {
        textaligncolum = 'R';
      }

      if (typedata == 'oid') {
        aux_theader += `<th>Zoom</th>`;
        textaligncolum = 'C';
      } else {
        aux_theader += `<th>${columname}</th>`;
      }
      aligncolum.push(textaligncolum);
    }
    theader = `<tr>${aux_theader}</tr>`;

    if (nreg != 0 && nfieldresponse != 0) {
      // armar cuerpo tbody
      for (let i = 0; i < nreg; i++) {
        const row = data[i].attributes;
        let aux_tbody = `<td>${i+1}</td>`;
        for (let j = 0; j < nfield; j++) {
          let item = row[fields[j].name];
          const typedata = fields[j].type;

          // alineacion de los campos
          textalign = 'text-left';
          if (['double', 'small-integer', 'integer', 'single'].includes(typedata)) {
            textalign = 'text-right';
          }

          if (item == undefined || item === null) {
            aux_tbody += `<td></td>`;
          } else {
            // dar formato a la fecha
            if (typedata == 'date') {
              item = getFormatDate(item, 'DD/MM/YYYY');
            }

            if (typedata == 'double') {
              item != null ? item = Number( item ).toFixed(2): '';
            }

            // solamente para campo con la palabra MES 
            if (fields[j].name == 'MES') {
              item = getMonthName(item).f;;
            }

            // asigno la zoom 
            if (typedata == 'oid') {
              aux_tbody += `<td class='tdzoom' id="${item}" data-toggle="tooltip" data-placement="left" title="Acercar"><span class='esri-icon-zoom-in-magnifying-glass' ></span></td>`;
            } else {
              aux_tbody += `<td class='${textalign}'>${item} </td>`;
            }
          }
        }
        tbody += `<tr>${aux_tbody}</tr>`;
      }
    } else {
      isexportable = false;
    }
    // luego llenar tabla con nueva data
    $(`${__idtable} > thead`).html(theader);
    $(`${__idtable} > tbody`).html(tbody);
    $(`${__idgrilla} .lbl-titlereporte`).text(titlereporte);

    // mostrar tabla
    showGrid();

    hidePreloader();
  }

  function getLengthScrollDataTable() {
    // obtengo el tamaño para tbody del datatable 
    const heightgrilla = $(__idgrilla).outerHeight();
    let heightgrillaheader = $(__idgrilla + ' .card-header-gis').outerHeight();
    let heighttabsbuffer = $(__idgrilla + ' .jcarousel').outerHeight();
    let heightdtheader = 38; // altura del header del datatable
    const heightdtfooter = 37; // altura del footer del datatable 
    heightgrillaheader < 28 ? heightgrillaheader = 34 : '';
    typeof heighttabsbuffer === 'undefined' ? heighttabsbuffer = 0 : '';
    $(`${ __idgrilla } table`).hasClass('tbl-pe') ? heightdtheader += 38 : '';

    const lengthscrolly = heightgrilla - (heightgrillaheader + heightdtheader + heightdtfooter + heighttabsbuffer);

    // btn maximizadores 
    $('#div_btnmaximizadores, #div_btnmaximizadoresresult').css('bottom', `${heightgrilla}px`);

    return lengthscrolly;
  }

  function getDataTableExport(titlereporte, lengthscrolly, aligncolum) {
    let filename = titlereporte.replace(/\ \/\ /g, '-');
    filename = filename.replace(': ', '-');
    filename = filename.replace(/\ /g, '_');

    let table = $(__idtable).DataTable({
      dom: 't<"dt-footer"<"dt-info"i><"dt-pag-length"pl>>',
      "language": getDataTableIdioma(),
      buttons: [{
        extend: 'csvHtml5',
        text: '<i class="icon-file-excel-o"></i> CSV',
        titleAttr: 'CSV',
        title: titlereporte,
        filename: filename,
        exportOptions: {
          columns: ':visible'
        }
      }, {
        extend: 'excelHtml5',
        text: '<i class="icon-file-excel-o"></i> XLS',
        titleAttr: 'XLS',
        title: titlereporte,
        filename: filename,
        exportOptions: {
          columns: ':visible'
        }
      }, {
        extend: 'pdfHtml5',
        text: '<i class="icon-file-pdf-o"></i> PDF',
        titleAttr: 'PDF, ocultar columnas innecesarias para evitar desborde en el documento.',
        title: titlereporte,
        filename: filename,
        pageSize: 'A4',
        customize: function (doc) {
          // orientacion del doc. 
          let tableNode;
          let auxlength = doc.content.length;
          for (i = 0; i < auxlength; ++i) {
            if (doc.content[i].table !== undefined) {
              tableNode = doc.content[i];
              break;
            }
          }
          let rowIndex = 0;
          let tableColumnCount = tableNode.table.body[rowIndex].length;
          if (tableColumnCount > 7) {
            doc.pageOrientation = 'landscape';
          }

          // style de doc
          doc.pageMargins = [20, 60, 20, 60];
          doc.styles.tableHeader.fontSize = 8;
          doc.styles.tableHeader.fillColor = "#075daa";
          doc.defaultStyle.fontSize = 8;
          // quito la propiedad *title* 
          doc.content.splice(0, 1);

          // header => logo   title de reporte
          doc['header'] = (function () {
            return {
              table: {
                widths: [150, '*', 150],
                body: [
                  [{
                      image: 'data:image/png;base64,',
                      width: 150,
                      border: [false, false, false, false]
                    },
                    {
                      text: titlereporte,
                      style: 'header',
                      fontSize: 12,
                      bold: true,
                      alignment: 'center',
                      border: [false, false, false, false]
                    },
                  ]
                ]
              },
              margin: [20, 20, 20, 0]
            }
          });

          // footer => fecha y hora   numeracion de paginas
          const jsDate =`${moment().format('DD/MM/YYYY') } Hora: ${ moment().format('HH:mm:ss') }`; 

          doc['footer'] = (function (page, pages) {
            return {
              columns: [{
                  alignment: 'left',
                  text: ['Fecha : ', {
                    text: jsDate.toString()
                  }]
                },
                {
                  alignment: 'right',
                  text: ['Pagina ', {
                    text: page.toString()
                  }, ' de ', {
                    text: pages.toString()
                  }]
                }
              ],
              margin: 20
            }
          });

          // Styling the table: create style object
          let objLayout = {};
          // Horizontal line width
          objLayout['hLineWidth'] = function (i) {
            return .0;
          };
          // Vertikal line width
          objLayout['vLineWidth'] = function (i) {
            return .5;
          };
          // Horizontal line color
          objLayout['hLineColor'] = function (i) {
            return '#BCBCBC';
          };
          // Vertical line color
          objLayout['vLineColor'] = function (i) {
            return '#BCBCBC';
          };
          // Left padding of the cell
          objLayout['paddingLeft'] = function (i) {
            return 4;
          };
          // Right padding of the cell
          objLayout['paddingRight'] = function (i) {
            return 4;
          };
          // Inject the object in the document
          doc.content[0].layout = objLayout;
        },
        exportOptions: {
          columns: ':visible'
        }
      }, {
        extend: 'colvis',
        text: '<i class="icon-add"></i>',
        columns: ':not(.noVis)'
      }],

      columnDefs: [{
          targets: 0,
          searchable: false,
          className: 'noVis'
        },
        {
          targets: 1,
          searchable: false,
          className: 'noVis'
        }
      ],
      "bDestroy": true,
      "retrieve": true,
      "scrollX": true,
      "scrollY": lengthscrolly,
      "drawCallback": function (settings) {
        $('ul.pagination').addClass("pagination-sm");
        $('[data-toggle="tooltip"]').tooltip({
          trigger: "hover"
        }); // para activar el tooltip
      }
    });

    // añado los botones a un div personalizado
    table.buttons().container().insertAfter(`${__idgrilla} .txt-dt-filtar`);

    // pongo un indice a la tabla
    table.on('order.dt search.dt', function () {
      table.column(0, {
        search: 'applied',
        order: 'applied'
      }).nodes().each(function (cell, i) {
        cell.innerHTML = i + 1; // asigno el número para el indice
        table.cell(cell).invalidate('dom'); //para exportar el indece de la tabla: para indicar a DataTables que vuelva a leer la información del DOM para la celda que contiene el índice. 
      });
    }).draw();

    setTimeout(() => {
      table.columns.adjust().draw(); // para ajustar el diseño de la tabla
    }, 500);
    
    return table;
  }

  function getDataTableNotExport(lengthscrolly) {
    let table = $(__idtable).DataTable({
      dom: 't<"dt-footer"<"dt-info"i><"dt-pag-length"pl>>',
      "language": getDataTableIdioma(),
      buttons: [{
        extend: 'colvis',
        text: '<i class="icon-add"></i>',
        columns: ':not(.noVis)'
      }],
      columnDefs: [{
          targets: 0,
          searchable: false,
          orderable: false,
          className: 'noVis'
        },
        {
          targets: 1,
          searchable: false,
          orderable: false,
          className: 'noVis'
        }
      ],
      "order": [
        [2, "desc"]
      ],
      "bDestroy": true,
      "retrieve": true,
      "scrollX": true,
      "scrollY": lengthscrolly,
      "drawCallback": function (settings) {
        $('ul.pagination').addClass("pagination-sm");
        $('[data-toggle="tooltip"]').tooltip({
          trigger: "hover"
        }); // para activar el tooltip
      }
    });

    // añado los botones a un div personalizado
    table.buttons().container().insertAfter(`${__idgrilla} .txt-dt-filtar`);

    // pongo un indice a la tabla 
    table.on('order.dt search.dt', function () {
      table.column(0, {
        search: 'applied',
        order: 'applied'
      }).nodes().each(function (cell, i) {
        cell.innerHTML = i + 1; // asigno el número para el indice
        table.cell(cell).invalidate('dom'); //para exportar el indece de la tabla: para indicar a DataTables que vuelva a leer la información del DOM para la celda que contiene el índice. 
      });
    }).draw();

    setTimeout(() => {
      table.columns.adjust().draw(); // para ajustar el diseño de la tabla
    }, 500);

    $(`${__idgrilla} .tbl-exportar`).css('min-width', 'auto');

    return table;
  }

  // Datatables para las tablas que no tienen el campo zoom 
  function getDataTable(lengthscrolly) {
    let table = $(__idtable).DataTable({
      dom: 't<"dt-footer"<"dt-info"i><"dt-pag-length"pl>>',
      "language": getDataTableIdioma(),
      buttons: [{
        extend: 'colvis',
        text: '<i class="icon-add"></i>',
        columns: ':not(.noVis)'
      }],
      columnDefs: [{
        targets: 0,
        searchable: false,
        className: 'noVis',
        orderable: false
      }],
      "bDestroy": true,
      "retrieve": true,
      "scrollX": true,
      "scrollY": lengthscrolly,
      "drawCallback": function (settings) {
        $('ul.pagination').addClass("pagination-sm");
        $('[data-toggle="tooltip"]').tooltip({  // para activar el tooltip
          trigger: "hover"
        });
      }
    });

    // añado los botones a un div personalizado
    __idgrilla === '' ? '' : table.buttons().container().insertAfter(`${__idgrilla} .txt-dt-filtar`);

    // pongo un indice a la tabla 
    table.on('order.dt search.dt', function () {
      table.column(0, {
        search: 'applied',
        order: 'applied'
      }).nodes().each(function (cell, i) {
        cell.innerHTML = i + 1; // asigno el número para el indice
        table.cell(cell).invalidate('dom'); //para exportar el indece de la tabla: para indicar a DataTables que vuelva a leer la información del DOM para la celda que contiene el índice. 
      });
    }).draw();

    setTimeout(() => {
      table.columns.adjust().draw(); // para ajustar el diseño de la tabla
    }, 500);

    return table;
  }

  // añado search de datatable a un input personalizado
  $(`${__idgrilla} .txt-dt-filtar`).on('keyup', function () {
    __datatable.search(this.value).draw();
  });

  // admin => datatable user
  function getDataTableUser() {

    const heightbody = $(window).height();
    const lengthscrolly = heightbody - 280;

    let table = $('#tbl_users').DataTable({
      dom: 't<"dt-footer"<"dt-info"i><"dt-pag-length"pl>>',
      "language": getDataTableIdioma(),
      "bDestroy": true,
      "retrieve": true,
      "scrollX": true,
      "scrollY": lengthscrolly,
      "drawCallback": function (settings) {
        $('ul.pagination').addClass("pagination-sm");
        $('[data-toggle="tooltip"]').tooltip({
          trigger: "hover"
        }); // para activar el tooltip
      },
      columnDefs: [{
        targets: 0,
        orderable: false,
      }]
    });

    // pongo un indice a la tabla 
    table.on('order.dt search.dt', function () {
      table.column(0, {
        search: 'applied',
        order: 'applied'
      }).nodes().each(function (cell, i) {
        cell.innerHTML = i + 1; // asigno el número para el indice
        table.cell(cell).invalidate('dom'); //para exportar el indece de la tabla: para indicar a DataTables que vuelva a leer la información del DOM para la celda que contiene el índice. 
      });
    }).draw();

    return table;
  }

  // admin => datatable roles 
  function getDataTableRoles() {
    const heightbody = $(window).height();
    const lengthscrolly = heightbody - 320;

    let table = $('#tbl_permisos').DataTable({
      dom: 't<"dt-footer"<"dt-info"i><"dt-pag-length"pl>>',
      "language": getDataTableIdioma(),
      "bDestroy": true,
      "retrieve": true,
      "scrollX": true,
      "scrollY": lengthscrolly,
      "drawCallback": function (settings) {
        $('ul.pagination').addClass("pagination-sm");
        $('[data-toggle="tooltip"]').tooltip({ // para activar el tooltip
          trigger: "hover"
        }); 
      },
      columnDefs: [{
        targets: 0,
        orderable: false,
      }]
    });

    // pongo un indice a la tabla 
    table.on('order.dt search.dt', function () {
      table.column(0, {
        search: 'applied',
        order: 'applied'
      }).nodes().each(function (cell, i) {
        cell.innerHTML = i + 1; // asigno el número para el indice
        table.cell(cell).invalidate('dom'); //para exportar el indece de la tabla: para indicar a DataTables que vuelva a leer la información del DOM para la celda que contiene el índice. 
      });
    }).draw();

    return table;
  }

  function getDataTableIdioma() {
    const idioma = {
      "sProcessing": "Procesando...",
      "sLengthMenu": " _MENU_ ",
      "sZeroRecords": "No se encontraron registros!",
      "sEmptyTable": "No se encontraron registros!",
      "sInfo": "_START_ - _END_ de _TOTAL_ resultados",
      "sInfoEmpty": "0 - 0 de 0 resultados",
      "sInfoFiltered": "(filtrado de un total de _MAX_ registros)",
      "sInfoPostFix": "",
      "sSearch": "Buscar:",
      "sUrl": "",
      "sInfoThousands": ",",
      "sLoadingRecords": "Cargando...",
      "oPaginate": {
        "sFirst": "<<",
        "sLast": ">>",
        "sNext": ">",
        "sPrevious": "<"
      },
      "oAria": {
        "sSortAscending": ": Activar para ordenar la columna de manera ascendente",
        "sSortDescending": ": Activar para ordenar la columna de manera descendente"
      },
      "buttons": {
        "copy": "Copiar",
        "colvis": "Visibilidad"
      }
    };
    return idioma;
  }



  function renderGraphic(response, _gl_search) {
    _gl_search.removeAll();
    const symbol = getTypeSymbol(response.geometryType);
    const auxlength = response.features.length;
    let graphics = [];

    for (let i = 0; i < auxlength; i++) {
      const item = response.features[i];
      const _gra_search = new Graphic({
        geometry: item.geometry,
        symbol: symbol,
      });
      graphics.push(_gra_search);
    }

    _gl_search.addMany(graphics);
  }

  function renderToZoom(response, _gl_search) {
    _gl_search.removeAll();
    const symbol = getTypeSymbol(response.geometryType);
    const auxlength = response.features.length;
    let graphics = [];

    for (let i = 0; i < auxlength; i++) {
      const item = response.features[i];
      const _gra_search = new Graphic({
        geometry: item.geometry,
        symbol: symbol,
      });
      graphics.push(_gra_search);
    }

    _gl_search.addMany(graphics);

    __globspace.view.when(function () {
      __globspace.view.goTo({
        target: _gl_search.graphics.toArray()
      });
    });
  }

  function renderToZoomBuffer(response, _gl_search, geometrybuffer) {
    _gl_search.removeAll();
    const symbol = getTypeSymbol(response.geometryType);
    const auxlength = response.features.length;
    let graphics = [];

    for (let i = 0; i < auxlength; i++) {
      const item = response.features[i];
      const _gra_search = new Graphic({
        geometry: item.geometry,
        symbol: symbol,
      });
      graphics.push(_gra_search);
    }
    _gl_search.addMany(graphics);

    __globspace.view.when(function () {
      __globspace.view.goTo({
        target: geometrybuffer
      });
    });
  }

  function paintToZoom(sql, url_servicio, _gl_search) {
    _gl_search.removeAll();
    let _queryt = new QueryTask({ url: url_servicio }),
      _qparams = new Query();

    _qparams.where = sql;
    _qparams.outFields = ['*'];
    _qparams.returnGeometry = true;
    _queryt.execute(_qparams).then(function (response) {
      const geometrytype = response.geometryType,
        symbol = getTypeSymbol(geometrytype),
        _gra_search = new Graphic({
          geometry: response.features[0].geometry,
          symbol: symbol,
        });

      _gl_search.add(_gra_search);

      __globspace.view.when(function () {
        if (geometrytype != 'point') {
          __globspace.view.goTo({
            target: _gl_search.graphics,
          });
        } else {
          __globspace.view.goTo({
            target: _gl_search.graphics,
            zoom: 19
          });
        }
      });
    }).catch(function (error) {
      hidePreloader();
      showError(error);

    });
  }

  function getTypeSymbol(geometrytype) {
    let symbol = '';
    switch (geometrytype) {
      case "point":
        symbol = {
          type: "simple-marker",
          outline: {
            color: [0, 250, 237, 0.6],
            width: 1.5
          }
        };
        break;

      case "polyline":
        symbol = {
          type: "simple-line",
          color: [0, 250, 237, 0.6],
          width: "2.5",
          outline: {
            color: "#00faed",
            width: 1.5
          }
        }
        break;

      case "polygon":
        symbol = {
          type: "simple-fill",
          color: [0, 250, 237, 0.6],
          outline: {
            color: "#00faed",
            width: 1.5
          }
        };
        break;

      default:
        break;
    }

    return symbol;

  }



  // tooltip mouse 
  let $tooltip = $('#tooltip_mouse'),
    popperinstance = null,
    virtualelement = {
      getBoundingClientRect: generateGetBoundingClientRect()
    };

  function generateGetBoundingClientRect(x = 0, y = 0) {
    return () => ({
      width: 0,
      height: 0,
      top: y,
      right: x,
      bottom: y,
      left: x
    });
  }

  function createTooltipInstructions() {
    $tooltip.addClass('visible').removeClass('notvisible');
    popperinstance = new Popper(virtualelement, $tooltip, {
      onCreate({ instance }) {
        document.onmousemove = ({ x, y }) => {
          virtualelement.getBoundingClientRect = generateGetBoundingClientRect(x, y);
          instance.scheduleUpdate();
        };
      }
    });
  }

  function hideTooltipInstructions() {
    $tooltip.addClass('notvisible').removeClass('visible');
    if (popperinstance) {
      popperinstance.destroy();
      popperinstance = null;
    }
  }


  // funciones html
  function showGrid() {
    $(__idgrilla).addClass('visible').removeClass('notvisible');
  }

  function hideGrid() {
    $(__idgrilla).addClass('notvisible').removeClass('visible');
  }

  function showPreloader() {
    $('#div_preloader').addClass('preloader').removeClass('preloader-none');
  }

  function hidePreloader() {
    $('#div_preloader').removeClass('preloader').addClass('preloader-none');
  }

  // Encriptación 
  function encryptWithAES(text) {
    const passphrase = 'osinerg_sein';
    return CryptoJS.AES.encrypt(text, passphrase).toString();
  }

  // Desencriptación 
  function decryptWithAES(ciphertext) {
    const passphrase = 'osinerg_sein';
    const bytes = CryptoJS.AES.decrypt(ciphertext, passphrase);
    return bytes.toString();
  }

  function getFormatDate(date, format) { //dar formato cuando se pasa como paremtro en milisegundos
    let fecha = moment(date);
    if (fecha.isValid() && date != '' && date != 0 && typeof date != 'undefined') {
      fecha = fecha.format(format); // fecha.add(5,'hours').format(format);
    } else if (typeof date == 'undefined')
      fecha = 'undefined';
    else
      fecha = '';
    return fecha;
  }

  function getFormatDatePE(date, format) { //dar formato cuando se pasa como paremtro en milisegundos para el layer Punto de Entrega
    let fecha = moment(date);
    if (fecha.isValid() && date != '' && date != 0 && typeof date != 'undefined') {
      fecha = fecha.add(5, 'hours').format(format); //fecha = fecha.format(format); 
    } else if (typeof date == 'undefined')
      fecha = 'undefined';
    else
      fecha = '';

    return fecha;
  }

  // Obtener el nombre del mes 
  function getMonthName(nummes) { // s: small  f: full
    nummes = parseInt(nummes);
    nombremeses = {
      1: {
        s: "Ene",
        f: "Enero"
      },
      2: {
        s: "Feb",
        f: "Febrero"
      },
      3: {
        s: "Mar",
        f: "Marzo"
      },
      4: {
        s: "Abr",
        f: "Abril"
      },
      5: {
        s: "May",
        f: "Mayo"
      },
      6: {
        s: "Jun",
        f: "Junio"
      },
      7: {
        s: "Jul",
        f: "Julio"
      },
      8: {
        s: "Ago",
        f: "Agosto"
      },
      9: {
        s: "Set",
        f: "Setiembre"
      },
      10: {
        s: "Oct",
        f: "Octubre"
      },
      11: {
        s: "Nov",
        f: "Noviembre"
      },
      12: {
        s: "Dic",
        f: "Diciembre"
      }
    }
    return nombremeses[nummes];
  }

  //para asegurarse la visualización de la capa en el view 
  function turnOnParentsLayer(layer, hasparent) {
    //evaluación ascendente (de Capa a Sector) 
    if (!hasparent) {
      layer.visible = true;
    } else {
      typeof (layer.parent.title) === "undefined" ? hasparent = false: hasparent = true;
      layer.visible = true;
      turnOnParentsLayer(layer.parent, hasparent);
    }
    return layer.visible;
  }


  // mostrar un error personalizado
  function showError(error) {
    let messaje = '';
    switch (error.message) {
      case 'Timeout exceeded':
        messaje = 'Se detectaron algunos inconvenientes de conexión!. Por favor vuelva a intentar.';
        break;
      case 'Error performing query operation':
        // messaje = 'Se detectaron algunos inconvenientes en el servidor!. Por favor vuelva a intentar.';
        messaje = 'Se detectaron algunos errores internos al ejecutar la consulta. Si el error persiste, por favor comuníquelo al administrador del sistema';
        break;
      default:
        messaje = 'Se detectaron algunos inconvenientes internos al ejecutar la consulta.';
        break;
    }

    alertMessage(messaje, 'warning', '', true);
    console.log("query task error");
    console.warn(error);
  }

  function getValidationForm(form, requiere) {
    let $form = $(`#${ form }`);
    let auxlength = requiere.length;

    let v_continue = true;
    let counterror = 0;
    $('.form-group', $form).removeClass('error');
    $('.form-group span.lbl-error', $form).remove();

    for (let i = 0; i < auxlength; i++) {
      const idfiel = requiere[i].idfiel;
      const label = requiere[i].label;

      if ($(`#${ idfiel }`).val().trim() === '') {
        $(`#${ idfiel }`).parent().addClass('error').append(`<span class=lbl-error> ${ label } obligatorio</span>`);
        $(`#${ idfiel }`).val('');
        counterror++;
      }

    }

    $('.error input:first', $form).focus();
    $('.error select:first', $form).focus();

    if (counterror != 0) {
      v_continue = false;
      hidePreloader();
    }
    return v_continue;
  }
  
  return {
    loadTable: function (response, titlereporte, idtable, isexportable) {
      return loadTable(response, titlereporte, idtable, isexportable);
    },
    loadTableTransform: function (response, titlereporte, idtable, isexportable) {
      return loadTableTransform(response, titlereporte, idtable, isexportable);
    },
    loadTableIndicador: function (response, titlereporte, idtable, isexportable) {
      return loadTableIndicador(response, titlereporte, idtable, isexportable);
    },
    loadTablePE: function (response, fields, idtable, isactionlayerlist) {
      return loadTablePE(response, fields, idtable, isactionlayerlist);
    },
    loadTablePESuministro: function (response, fields, idtable) {
      return loadTablePESuministro(response, fields, idtable);
    },

    getDataTableUser: function () {
      return getDataTableUser();
    },
    getDataTableRoles: function () {
      return getDataTableRoles();
    },

    renderToZoom: function (response, _gly_search) {
      return renderToZoom(response, _gly_search);
    },
    renderGraphic: function (response, _gly_search) {
      return renderGraphic(response, _gly_search);
    },
    renderToZoomBuffer: function (response, _gly_search, geometrybuffer) {
      return renderToZoomBuffer(response, _gly_search, geometrybuffer);
    },
    paintToZoom: function (sql, url_servicio, _gly_search) {
      return paintToZoom(sql, url_servicio, _gly_search);
    },

    showGrid: function () {
      return showGrid();
    },
    hideGrid: function () {
      return hideGrid();
    },
    showPreloader: function () {
      return showPreloader();
    },
    hidePreloader: function () {
      return hidePreloader();
    },
    createTooltipInstructions: function () {
      return createTooltipInstructions();
    },
    hideTooltipInstructions: function () {
      return hideTooltipInstructions();
    },

    encryptWithAES: function (text) {
      return encryptWithAES(text);
    },
    decryptWithAES: function (ciphertext) {
      return decryptWithAES(ciphertext);
    },

    getFormatDate: function (date, format) {
      return getFormatDate(date, format);
    },
    getFormatDatePE: function (date, format) {
      return getFormatDatePE(date, format);
    },
    getMonthName: function (nummes) {
      return getMonthName(nummes);
    },

    turnOnParentsLayer: function (layer, hasparent) {
      return turnOnParentsLayer(layer, hasparent);
    },
    showError: function (error) {
      return showError(error);
    },
    getFieldsHide: function () {
      return __arr_fieldhide;
    },
    getValidationForm: function (form, requiere) {
      return getValidationForm(form, requiere);
    },

  }

});