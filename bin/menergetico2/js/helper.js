define([
    ...
    function loadTableByFindSensitivityCompany(response, fields, titlereporte, idtable, isexportable) {
        __idgrilla = '#' + $(idtable).parents('.grilla').attr('id');
        __idtable = idtable;
        $('#div_results .grilla').addClass('notvisible');
        hideGrid();
        ...
        let data = response.features,
            ...
            aligncolum = [],
            textalign = '';
        let code = data[0].attributes.CODEMP;
        let nameEmpresa = "";
        $.ajax({
            type: "GET",
            url: "https://gisem.osinergmin.gob.pe/serverosih/rest/services/Electricidad/ELECTRICIDAD/MapServer/30?f=json",
            success :  function(data){
                data.fields[2].domain.codedValues.forEach(f=>{
                    if(f.code == code){
                        nameEmpresa = f.name;
                    }
                });
            ...
            function(){
                ...
                for (let j = 0; j < nfield; j++) {
                    ...
                    if ($.inArray(fields[j].name, __arr_fieldhide) == -1) { 
                        ...
                        if(typedata == 'oid'){
                            aux_theader += `<th>Zoom</th>`;
                            aux_theader += `<th>ETIQUETA EMPRESA</th>`;
                            textaligncolum = 'C';
                        }else{
                            aux_theader += `<th>${columname}</th>`;
                        }
                            ...
                            if ($.inArray(fields[j].name, __arr_fieldhide) == -1) {
                                ...
                                if (item == undefined || item === null) {
                                    ...
                                    if (typedata == 'oid') {
                                        ...
                                        if(j==2){
                                            aux_tbody += `<td class='${textalign}'>${nameEmpresa} </td>`;
                                        }
                                        aux_tbody += `<td class='${textalign}'>${item} </td>`;
    ...
    return {
        ...
        loadTableByFindSensitivityCompany: function(response, queryfields,titlereporte, url_servicio, isexportable){return loadTableByFindSensitivityCompany(response, queryfields, titlereporte, url_servicio, isexportable); },
