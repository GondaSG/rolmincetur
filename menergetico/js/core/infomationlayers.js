define([
    ...
    _lyl_hidrocarburos.opacity = 0.75;
    ...
    watchUtils.whenFalse(__globspace.view, "updating", function(evt) {
        ...
        if ($('body').hasClass("cargando")) { //acciones para el primer updating solamente (carga inicial)
            ...
            $('#close_msjwelcome').removeClass('d-none').addClass('d-block');
    ...
    $($divmenu).on('click', '.chb-sectormenu', function() {
        ...
        let $containerlyl = $($(this).siblings(".btn-sectormenu").attr('data-subcontainer'));
        if (isactive) {
            let top = $containerlyl.attr('data-top');
            $containerlyl.removeClass('notvisible').addClass('visible');
            $containerlyl.css('top', top);
        } else {
            $containerlyl.removeClass('visible').addClass('notvisible');
        }
    ...
    $($divmenu).on('click', '.lbl-sectormenu', function() {
        ...
        switch (itemchb) {
            ...
            case 'lbl_sectorgasnatural':
                ...
                __mil_gasnatural.visible = isactive;
                break;
            case 'lbl_sectorhidrocarburos':
                $("#chb_sectorhidrocarburos").addClass("active");
                __mil_hidrocarburos.visible = isactive;
                break;
            case 'lbl_sectormineria':
                $("#chb_sectormineria").addClass("active");
                __mil_mineria.visible = isactive;
                break;
            case 'lbl_sectorfep':
                $("#chb_sectorfep").addClass("active");
                __mil_fep.visible = isactive;
                break;
            case 'lbl_sectoraue':
                $("#chb_sectoraue").addClass("active");
                __mil_aue.visible = true;
                break;
            case 'lbl_sectoraniadido':
                $("#chb_sectoraniadido").addClass("active");
                __gru_aniadido.visible = isactive;
                break;
        }
    });
    ...
    $($divmenu).on('click', '.btn-sectormenu', function() {
        ...
        let itemchb = $(this).siblings(".chb-sectormenu");
        let isactive = $(itemchb).hasClass("active");
    ...
    function populationChange(feature) {
        ...
        //node.children[0].children[0].children[0].append($(content))
        return node;
    };
    ...
    $("#btnswitch_2d3d").on('click', function() {
        ...
        if (!valor) {
            ...
            //$("span.esri-layer-list__item-title").hide();
            //$(".esri-layer-list__item .esri-layer-list__item-container").hide();
            /*$("#lyl_electricidad ul li:nth-child(2) .esri-layer-list__item .esri-layer-list__item-container .esri-layer-list__item-label").hide();
            $("#lyl_gasnatural ul li:nth-child(2)").hide();
            $("#lyl_hidrocarburos ul li:nth-child(2)").hide();
            $("#lyl_mineria ul li:nth-child(2)").hide();
            $("#lyl_fep ul li:nth-child(2)").hide();
            $("#lyl_aue ul li:nth-child(2)").hide();*/
            //console.log("pasar a 3D");
        }

        __map.addMany([__globspace._gly_searchadvanced,
            __globspace._gly_searchinlayer,
            __globspace._gly_coodxy,
            __mil_aue,
            __mil_fep,
            __mil_mineria,
            __mil_hidrocarburos,
    ...
    $('#wg_addlayer').on('click', '#btn_clear_addlayer', function() {
        ...
        $('#fil_upkml').val("");
        //__gru_aniadido.layers.re
        __gru_aniadido.layers.removeAll();
        legend.layerInfos = { layer: '' };
    });
    //Cerrar widget
    $('#btn_clear_addlayer_icon_close').on('click', function() {
        $('#txt_urlservice').val("");
        $('#sp_uploadstatus_url').html("");
        $('#sp_uploadstatus_shp').html("");
        $('#fil_upshp').val("");
    ...
    function loadView3D(view) {
        watchUtils.whenFalse(view, "updating", function(evt) {
            ...
            $("#lyl_electricidad > ul:nth-child(1) > li:nth-child(2)").hide();
            $("#lyl_gasnatural > ul:nth-child(1) > li:nth-child(2)").hide();
            $("#lyl_hidrocarburos > ul:nth-child(1) >  li:nth-child(2)").hide();
            $("#lyl_mineria > ul:nth-child(1) > li:nth-child(2)").hide();
            $("#lyl_fep > ul:nth-child(1) > li:nth-child(2)").hide();
            $("#lyl_aue > ul:nth-child(1) > li:nth-child(2)").hide();
        });
    }
