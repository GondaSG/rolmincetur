...
var geoconv = function(e) {
	...
	self.dec2gms = function (valor, tipo) {//{{{
		...
		return {
			...
			'segundos' : segundos,
			'direccion': direccion,
			//'valor'    : grados + "\u00b0 " + minutos + "' "+ segundos.toFixed(4) + "\"" + ((isNaN(direccion)) ? (' ' + direccion) : '')
            'valor'    : grados + "\u00b0 " + minutos + "' "+ segundos.toFixed(4) + "\""
		};
	};//}}}

    self.gms2dec = function (grados, minutos, segundos, direccion) {//{{{
		if(direccion)
		{
